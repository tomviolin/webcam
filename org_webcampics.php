...
function exifdate_from_filename($file)
{
...
function stampfile($src_filename, $dest_filename)
{
	...
	if (! $raw_source_image) { 
		print("ERROR retrieving staging/$src_filename, ");
		print("pwd="+getcwd()+"\n");
	}
...
function process_one_file($filename) {
	...
	if ($linkit) {
		...
		// now an unfortunate kludge: make webcam image available to staging/out/current.jpg
		copy("currentlink.jpg","staging/out/staged.jpg");
		rename("staging/out/staged.jpg","staging/out/current.jpg");
	}
}
