...
umask(002);
$factor = 2560/1920;
system("find /home/tomh/iphonepics/staging/ -size -50k -exec rm {} +");
$USER=chop(`whoami`);
echo "user: $USER\n";
...
$factor = 2560/1920;
	...
	// 	int $dst_w , int $dst_h , 
	// 	int $src_w , int $src_h ) 
	$ip = json_decode(file_get_contents("/home/tomh/bin/imageparams.json"));
	print_r($ip);
...
while (true) {
	sleep(0.1);
	echo ("cd /home/$USER/iphonepics/staging\n");
	chdir("/home/$USER/iphonepics/staging");
...
//	print_r($files);
	...
	foreach ($files as $f) {
		...
		$day=substr($f,6,2);
		$hour=substr($f,9,2);
		echo "mkdir /home/$USER/iphonepics/tl/$year/$month/$day/$hour\n";
		$stampedfiledir="/home/$USER/iphonepics/tl/$year/$month/$day/$hour";
